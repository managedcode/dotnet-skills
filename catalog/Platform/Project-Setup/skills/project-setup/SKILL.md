...
- `.NET SDK 10.0.400` is the current 10.0.4xx feature band. It expands file-based app support, adds Microsoft.Testing.Platform `--project` and `--solution` selection to `dotnet test`, accepts `@` as the `dotnet new` option separator, and fixes hardlink and incremental-build paths. Keep `global.json`, workloads, and CI images explicit for the selected feature band.
