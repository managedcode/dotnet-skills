...
- Boundary: Microsoft Agent Framework is linked from this docs tree, but its dedicated authored snapshot and deeper routing guidance live in the separate `dotnet-microsoft-agent-framework` skill
...
- For orchestration, threads, workflows, or hosted-agent protocols, switch to the `dotnet-microsoft-agent-framework` skill rather than assuming the answer lives in the `Microsoft.Extensions.AI` layer.
