...
- `dotnet/extensions` `v10.8.4` removes GitHub Models from the AI Chat Web and AI Agent Web API templates. Both templates now require an explicit `--provider azureopenai`, `--provider ollama`, or `--provider openai`; update scaffolding scripts and CI smoke tests instead of relying on the old default.
- The `v10.8.4` templates also update Aspire Hosting and SQLite vector-data dependencies. Re-run generated-app startup, ingestion, and provider-authentication tests when refreshing a template-derived solution; runtime MEAI composition remains on the behavior described below.
- In `dotnet/extensions` `v10.8.1`, approval-gated functions now preserve tool-call/tool-result ordering when service-managed chat history resumes, and stateless OpenAI Responses flows preserve encrypted reasoning item ids. Re-run approval-resume and `store=false` reasoning round-trip tests after upgrading.
